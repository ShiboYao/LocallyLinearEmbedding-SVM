...
if __name__ == "__main__":
    ...
    for key in data_dic:
        ...
        plt.subplot(1,2,1)
        plt.title('LLE')
        plt.scatter(comp_LLE[:,0], comp_LLE[:,1], c=y, s=10, alpha=0.6, linewidth='0')
        plt.xticks([],[])
        plt.yticks([],[])
        comp_PCA = PCA(n_components=d).fit_transform(X)
        plt.subplot(1,2,2)
        plt.title('PCA')
        plt.scatter(comp_PCA[:,0], comp_PCA[:,1], c=y, s=10, alpha=0.6, linewidth='0')
        plt.yticks([],[])
        plt.xticks([],[])
        fig.savefig(key+'.pdf')
        print('\n')
    ...
    l1 = int(len(twin)*.5)
    l2 = int(len(twin)*0.75)
    ax.scatter(twin[l1:l2,1], twin[l1:l2,0], twin[l1:l2,2], c='red', linewidth='1')
    ax.scatter(twin[:l1,1], twin[:l1,0], twin[:l1,2], c='blue', linewidth='1')
    ax.scatter(twin[l2:,1], twin[l2:,0], twin[l2:,2], c='red', linewidth='1')
